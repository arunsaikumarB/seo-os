...
export function generateContentPack(
  ...
  const pack: ContentPackPayload = {
    ...
    body,
    shortDescription: `${brandName} provides ${industry} solutions for audiences on ${oppCtx.website_name ?? oppCtx.domain ?? 'partner sites'}.`,
    longDescription: body.slice(0, 1200),
    businessDescription: `${brandName} delivers ${industry} via https://${domain}. Tailored for ${oppCtx.website_name ?? oppCtx.domain ?? 'this listing'} with practical expertise.`,
