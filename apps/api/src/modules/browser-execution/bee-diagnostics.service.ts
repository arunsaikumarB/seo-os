...
export async function getQueueMonitor(workspaceId: string) {
  ...
  return {
    ...
    waitingLogin: stats.waitingLogin ?? 0,
    waitingMfa: stats.waitingMfa ?? 0,
    waitingApproval: stats.waitingApproval ?? 0,
    waitingVerification: stats.waitingVerification ?? 0,
    averageRuntimeMs: stats.avgRuntimeMs,
    ...
    activeWorkerCount: stats.activeWorkerCount ?? 0,
    workers: stats.workers ?? [],
    browserPool: stats.browserPool ?? null,
    totalJobs: stats.totalJobs ?? 0,
    completedJobs: stats.completedJobs ?? 0,
    remainingJobs: stats.remainingJobs ?? 0,
    progressPercent: stats.progressPercent ?? 0,
    executionComplete: stats.executionComplete ?? false,
    estimatedApprovalTime: stats.estimatedApprovalTime ?? '7–14 days',
